...
stim = lum_obj
resp_y = np.roll(stim,+10)
resp_z = np.roll(stim,+50)
...
mdl_cnn = model.models.CNN2D(inputs,n_out,**dict_params)
...
plt.plot(mdl_cnn_history.history['fraction_of_explained_variance'][5:]);plt.show()
...
print('FEV_train_cnn = %0.2f' %fev_train_cnn)
...
plt.plot(mdl_adapt_history.history['fraction_of_explained_variance'][5:]);plt.show()
